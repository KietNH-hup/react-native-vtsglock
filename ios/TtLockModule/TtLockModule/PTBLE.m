...
// @ahmad
- (void)setAdminKeyBoardPassword:(NSString *)adminPS key:(LockModel *)key completion:(BLECompletion)completion {

    _commandDict[@(BLECommandSetAdminKeyboardPassword)] = completion;
    _currentKey = key;
    [_ttlock setAdminKeyBoardPassword:adminPS
                              adminPS:key.adminPwd
                              lockKey:key.lockKey
                               aesKey:key.aesKeyStr
...
// ahmad
- (void)setLockTimeValue:(long long)timeValue timezoneRawOffset:(long long)timezoneRawOffset key:(LockModel *)key completion:(BLECompletion)completion {

    if (timeValue > 0) {
        _commandDict[@(BLECommandSetTime)] = completion;
        _currentKey = key;

        [_ttlock setLockTime_lockKey:key.lockKey aesKey:key.aesKeyStr version:key.version unlockFlag:key.lockFlagPos.intValue referenceTime:[NSDate dateWithTimeIntervalSince1970:timeValue/1000] timezoneRawOffset:timezoneRawOffset];
    }else{
...
// ahmad
- (void)unlockByAdministrator:(LockModel *)key completion:(BLECompletion)completion {
    _commandDict[@(BLECommandUnlockByAdministrator)] = completion;
    _currentKey = key;

    _uniqueid = @([NSDate date].timeIntervalSince1970);

    [_ttlock unlockByAdministrator_adminPS:key.adminPwd
                                   lockKey:key.lockKey
                                    aesKey:key.aesKeyStr
                                   version:key.lockVersion
                                unlockFlag:key.lockFlagPos.intValue
                                  uniqueid:_uniqueid
                         timezoneRawOffset:key.timezoneRawOffset.intValue
     ];
}

// ahmad
- (void)resetKeyboardPassword:(LockModel *)key completion:(BLECompletion)completion {
    _commandDict[@(BLECommandResetKeyboardPassword)] = completion;
    _currentKey = key;

    [_ttlock resetKeyboardPassword_adminPS:key.adminPwd
                                   lockKey:key.lockKey
                                    aesKey:key.aesKeyStr
                                   version:key.lockVersion
                                unlockFlag:key.lockFlagPos.intValue
...
// ahmad
- (void)getLockAudioState:(LockModel *)key completion:(BLECompletion)completion{
    _currentKey = key;
    _commandDict[@(BLECommandGetAudioState)] = completion;
    [_ttlock operateAudioSwitch_type:OprationTypeQuery
                                isOn:true
                             adminPS:key.adminPwd
                             lockKey:key.lockKey
                              aesKey:key.aesKeyStr
...
// ahmad
- (void)setLockAudioState:(BOOL)isOn key:(LockModel *)key completion:(BLECompletion)completion{
    _currentKey = key;
    _commandDict[@(BLECommandSetAudioState)] = completion;
    [_ttlock operateAudioSwitch_type:OprationTypeModify
                                isOn:isOn
                             adminPS:key.adminPwd
                             lockKey:key.lockKey
                              aesKey:key.aesKeyStr
...
// ahmad
- (void)addPeriodKeyboardPassword:(NSString *)pin
                   startTimestamp:(double)startTimestamp
                     endTimestamp:(double)endTimestamp
                              key:(LockModel *)key
                       completion:(BLECompletion)completion{
    _currentKey = key;
    _commandDict[@(BLECommandAddPeriodKeyboardPassword)] = completion;
    ...
    [_ttlock addKeyboardPassword_password:pin
                                startDate:startDate
                                  endDate:endDate
                                  adminPS:key.adminPwd
                                  lockKey:key.lockKey
...
- (void)onAddUserKeyBoardPassword {
     BLECompletion completion = _commandDict[@(BLECommandAddPeriodKeyboardPassword)];
     if (completion) {
         [_commandDict removeObjectForKey:@(BLECommandAddPeriodKeyboardPassword)];
         dispatch_main_async(^{
             completion(true,nil);
         });
     }
...
// ahmad
- (void)deleteOneKeyboardPassword:(NSString *)pin
                                   key:(LockModel *)key
                            completion:(BLECompletion)completion{
         _currentKey = key;
         _commandDict[@(BLECommandDeleteOneKeyboardPassword)] = completion;


         [_ttlock deleteOneKeyboardPassword:pin
                                   adminPS:key.adminPwd
                                    lockKey:key.lockKey
                                    aesKey:key.aesKeyStr
 ...
 - (void)OnDeleteUserKeyBoardPassword {
      BLECompletion completion = _commandDict[@(BLECommandDeleteOneKeyboardPassword)];
      if (completion) {
          [_commandDict removeObjectForKey:@(BLECommandDeleteOneKeyboardPassword)];
          dispatch_main_async(^{
              completion(true,nil);
          });
      }
...
- (void)TTManagerDidUpdateState:(TTManagerState)state {
    if (state == TTManagerStatePoweredOff) {
        ...
        [_commandDict enumerateKeysAndObjectsUsingBlock:^(NSNumber*  _Nonnull command, BLECompletion  _Nonnull completion, BOOL * _Nonnull stop) {
            NSError *error = [NSError errorWithDomain:@"PTBLE" code:0x123f userInfo:@{NSLocalizedDescriptionKey: @"Bluetooth is off, please restart Bluetooth"}];
            dispatch_main_async(^{
...
//    NSDictionary *advertisementData = infoDic[@"advertisementData"];
    BOOL isAllowUnlock = [infoDic[@"isAllowUnlock"] boolValue];
     ...
     NSInteger electricQuantity = [infoDic[@"electricQuantity"] integerValue];
//    BOOL isDfuMode = [infoDic[@"isDfuMode"] boolValue];
    ...
    if (_commandDict[@(BLECommandScan)]) {//添加锁
        if (mac.length && protocolCategory == 5) {
            [self scanPeripheral:peripheral mac:mac rssi:rssi electricQuantity:electricQuantity lockName:lockName isContainAdmin:isContainAdmin isTouch:isAllowUnlock protocolType:protocolType protocolVersion:protocolVersion];
        }
    }
}
...
- (void)onBTDisconnect_peripheral:(CBPeripheral *)periphera {
    ...
    dispatch_main_async(^{
        [self.commandDict enumerateKeysAndObjectsUsingBlock:^(NSNumber*  _Nonnull command, BLECompletion  _Nonnull completion, BOOL * _Nonnull stop) {
            if (command.intValue != BLECommandScan) {
                completion(false,@"Bluetooth connection is interrupted, please try again");
            }
        }];
...
//            });
    ...
    BLECompletion completion2 = _commandDict[@(BLECommandUnlockByAdministrator)];
    if (completion2) {
        [_commandDict removeObjectForKey:@(BLECommandUnlockByAdministrator)];
        dispatch_main_async(^{
            completion2(true,nil);
        });
    }
...
- (void)onQueryAudioSwitch:(BOOL)isOn {
    BLECompletion completion = _commandDict[@(BLECommandGetAudioState)];
    if (completion) {
        [_commandDict removeObjectForKey:@(BLECommandGetAudioState)];
...
- (void)onModifyAudioSwitch {
    BLECompletion completion = _commandDict[@(BLECommandSetAudioState)];
    if (completion) {
        [_commandDict removeObjectForKey:@(BLECommandSetAudioState)];
        dispatch_main_async(^{
            completion(true,nil);
        });
    }
...
- (void)onSetAdminKeyboardPassword{
    BLECompletion completion = _commandDict[@(BLECommandSetAdminKeyboardPassword)];
    if (completion) {
        [_commandDict removeObjectForKey:@(BLECommandSetAdminKeyboardPassword)];
        dispatch_main_async(^{
            completion(true,nil);
        });
    }
...
-(void)onResetKeyboardPassword_timestamp:(NSString *)timestamp pwdInfo:(NSString *)pwdInfo{
    BLECompletion completion = _commandDict[@(BLECommandResetKeyboardPassword)];
    if (completion) {
        [_commandDict removeObjectForKey:@(BLECommandResetKeyboardPassword)];
        dispatch_main_async(^{
            completion(true,nil);
        });
    }
...
- (void)scanPeripheral:(CBPeripheral *)peripheral mac:(NSString *)mac rssi:(NSString *)rssi electricQuantity:(NSInteger)electricQuantity lockName:(NSString *)lockName isContainAdmin:(BOOL)isContainAdmin isTouch:(BOOL)isTouch protocolType:(NSString *)protocolType protocolVersion:(NSString *)protocolVersion{
...
//
    ...
    if (lockName && mac) {
        ...
        model.electricQuantity = electricQuantity;
...
- (NSError *)errorWithCode:(TTError)code {
    NSMutableString *errorString = @"Operation failed ".mutableCopy;
        switch (code) {
         case TTErrorHadReseted:
        [errorString appendString:@"Lock may have been reset"];
        break;
        case TTErrorDataCRCError:
        [errorString appendString:@"CRC error"];
        break;
        case TTErrorNoPermisston:
        [errorString appendString:@"No permission"];
        break;
   case TTErrorIsWrongPS:
          [errorString appendString:@"Wrong ps"];
          break;
      case TTErrorNoMemory:
          [errorString appendString: @"lock memory space is not enough"];
          break;
       case TTErrorInSettingMode:
            [errorString appendString:@"In setting mode"];
           break;
       case TTErrorNoAdmin:
           [errorString appendString:@"Administrator does not exist"];
            break;
        case TTErrorIsNotSettingMode:
             [errorString appendString:@"Not setting mode"];
           break;
        case TTErrorIsWrongDynamicPS:
           [errorString appendString:@"Wrong dynamic ps"];
           break;
       case TTErrorIsNoPower:
           [errorString appendString:@"Lock is low"];
            break;
        case TTErrorResetKeyboardPs:
            [errorString appendString:@"Reset keyboard ps"];
             break;
        case TTErrorUpdateKeyboardIndex:
             [errorString appendString:@"Update keyboard index"];
             break;
         case TTErrorIsInvalidFlag:
             [errorString appendString:@"The electronic key has been reset"];
             break;
         case TTErrorEkeyOutOfDate:
             [errorString appendString:@"The electronic key has expired"];
             break;
         case TTErrorPasswordLengthInvalid:
             [errorString appendString:@"The password length is invalid"];
             break;
         case TTErrorSuperPasswordIsSameWithDeletePassword:
             [errorString appendString:@"admin password is equal to delete password"];
             break;
         case TTErrorEkeyNotToDate:
             [errorString appendString:@"The electronic key has not expired"];
             break;
         case TTErrorAesKey:
            [errorString appendString:@"AES key"];
             break;
         case TTErrorPasswordNotExist:
             [errorString appendString:@"Password does not exist"];
             break;
         case TTErrorPsswordExist:
             [errorString appendString:@"Password already exists"];
             break;
         case TTErrorNoFree_Memory:
             [errorString appendString:@"Insufficient memory space"];
             break;
         case TTErrorCardNotExist:
             [errorString appendString:@"IC card does not exist"];
             break;
         case TTErrorFingerprintDuplication:
             [errorString appendString:@"fingerprint repeat"];
             break;
         case TTErrorFingerprintNotExist:
             [errorString appendString:@"fingerprint does not exist"];
             break;
         Default:
             break;
    }
